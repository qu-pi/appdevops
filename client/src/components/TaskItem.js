  ...
  return `
    <li class="task-item priority-${task.priority} ${task.completed ? 'is-completed' : ''} ${overdue ? 'is-overdue' : ''}" data-id="${task.id}">
      <label class="task-check">
        <input type="checkbox" ${task.completed ? 'checked' : ''} data-action="toggle" aria-label="${t('toggleLabel')}" />
      ...
      <div class="task-body">
        <div class="task-title-row">
          ...
          <span class="badge badge-${task.priority}">${t(PRIORITY_KEYS[task.priority])}</span>
          ${task.category ? `<span class="badge badge-category">${escapeHtml(task.category)}</span>` : ''}
        </div>
        ${task.description ? `<p class="task-description">${escapeHtml(task.description)}</p>` : ''}
        ${
          task.dueDate
            ? `<span class="task-due">${overdue ? `⚠️ ${t('overdue')}` : '📅'} ${formatDate(task.dueDate, getEffectiveLocale())}</span>`
            : ''
        }
      </div>
      <div class="task-actions">
        <button type="button" class="icon-btn" data-action="delete" aria-label="${t('deleteLabel')}">
